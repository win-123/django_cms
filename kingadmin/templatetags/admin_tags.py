...
from django.utils.safestring import mark_safe, EscapeString
            ...
            <div class="searchbox">
               <form method="get">
                    <div class="input-group">
                        <input type="text" style="width:170px;" name="q" value='%s' class="form-control" placeholder="%s"/>
                        %s
                        <span style="display: inline-block;width:32px;height: 25px;margin:6px 0 0 20px;">
                            <label for="for_sub"><i class="fa fa-search"></i></label>
                            <input class="text-muted hide" id="for_sub" type="submit"/>
                        </span>
                    </div>
               </form>
           </div>
...
def render_list_editable_column(table_obj, row_obj, field_obj):
    ...
    if field_obj.get_internal_type() in ("CharField", "ForeignKey", "BigIntegerField", "IntegerField","FloatField",
        ...
        if not field_obj.choices and field_obj.get_internal_type() != "ForeignKey":
            ...
            column = '''<input ele-tag='editable' data-tag='%s' type='text' name='%s' value='%s' >''' % \
                     (field_obj.name, field_obj.name, '' if field_obj._get_val_from_obj(row_obj) is None else field_obj._get_val_from_obj(row_obj))

        else:
            column = '''
            <i style="cursor: pointer" data-target="#modal-dialog" data-toggle="modal" class="fa fa-search" aria-hidden="true" onclick="PrepareFilterSearch('%s_%s')"></i>
            <select ele-tag='editable' data-tag="%s_%s" class='form-control'  name='%s' >''' % (row_obj.id,
                                                                                                field_obj.name,
                                                                                                row_obj.id,
                                                                                                field_obj.name,
                                                                                                field_obj.name)
        ...
        if column_data == True:
            ...
            checked = ''
        column = '''<input ele-tag='editable'  type='checkbox' name='%s' value="%s"  %s> ''' % (field_obj.name,
                                                                                                column_data,
                                                                                                checked)
            ...
            if hasattr(row_obj, column_name):
                ...
                if onclick_column == column_name:
                    ...
                    column = '''<td><a class='btn-link'  href='%schange/%s/' >%s</a> </td> ''' \
                             % (table_obj.request.path, row_obj.id, column_data)
                    ...
                    if type(column_data) is str and len(column_data) > 300:
                        column_data = column_data[:300]
                    column = "<td>%s</td>" % column_data
                ...
                raise KeyError("cannot find column %s in model %s" % (column_name, table_obj.model_class))
            row_ele += column
    else:
...
def get_m2m_objs(rel_field_name, form_obj):
    ...
    try:
        ...
        limit_choices_dic = m2m_field_obj.field.get_limit_choices_to()
        return m2m_field_obj.rel.to.objects.filter(**limit_choices_dic)
    ...
    if field_type in ("ForeignKey", "ManyToManyField"):
        ...
        if app_label in site.enabled_admins:
            if model_name in site.enabled_admins.get(app_label):  # make sure this class is registered
                ...
                if field.name not in form_obj.Meta.admin.readonly_fields and not form_obj.Meta.admin.readonly_table:
                    popup_window = '/kingadmin/{app}/{model}/add/?_popup=4&_to_field={field_name}'.format(
...
@register.simple_tag
def get_rendered_field(field_name,form_obj):
    """
    根据field name从已经渲染好的form_obj中得到对应的field对象
    :param field_name: 
    :param form_obj: 
    :return: 
    """

    #if hasattr(form_obj.fields,field_name):
    field_obj = form_obj.fields.get(field_name)
    boundField = forms.forms.BoundField(form_obj, field_obj, field_name)
...
@register.simple_tag
def add_date_time_select(form_obj, field):
    """
    增加对 datetimefield 字段的处理
    :param form_obj:
    :param field:
    :return:
    """
    field_obj = form_obj.instance._meta.get_field(field.name)
    if "DateTimeField" == field_obj.get_internal_type():
        ele = '<br class ="clear"><input type="text" name=%s_0 placeholder="" class="vTimeField datetime" ' \
              'type="text" size="8" id = id_%s_0 />' % (field.name, field.name)
...
def add_fk_search_btn(form_obj, field):
    """
    在kingadmin 表数据修改页为每个fk关联加上fk_search按钮
    :param form_obj: 
    :param field: 
    :return: 
    """
    ...
    if field_obj.get_internal_type() in  ("ForeignKey","OneToOneField"):
        ...
        if field.name not in form_obj.Meta.admin.readonly_fields and not form_obj.Meta.admin.readonly_table:
            ...
            return mark_safe(ele)
    return '' # field is not fk or one2one
...
@register.simple_tag
def native_column_check(column, table_obj):
